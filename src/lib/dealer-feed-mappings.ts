...
const KODLLIN: FeedPreset = {
  ...
  // Kodllin uses camelCase field names. stockNo is present on all Kodllin feeds
  // (lab and natural). growthType is lab-only so we can't require it.
  detect: (row) => "stockNo" in row && ("reportNo" in row || "imageLink" in row || "pricePerCt" in row),
  map: (row) => {
