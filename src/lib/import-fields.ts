...
export const CLARITY_VALUES = [
  ...
  "Eye Clean", "Lightly Included", "Moderately Included", "Heavily Included",
];

export const CERT_LABS = [
  "GIA", "IGI", "HRD", "AGS", "GCAL", "EGL",
  "GRS", "SSEF", "Gübelin", "AGL", "Lotus", "GIT", "GIA-coloured",
];

export const STONE_FIELDS: StoneField[] = [
  { key: "stone_type", label: "Stone type", type: "string", required: true, aliases: ["type", "gem", "gemstone", "stone"] },
  { key: "shape", label: "Shape", type: "string", required: true, aliases: ["cut shape", "outline"] },
  { key: "carat_weight", label: "Carat weight", type: "number", required: true, aliases: ["weight", "carats", "ct", "carat", "carat_wt"] },
  { key: "wholesale_price_usd", label: "Wholesale price (USD)", type: "number", required: true, aliases: ["price", "price_usd", "wholesale", "total", "amount"] },
  { key: "colour_grade", label: "Colour grade", type: "string", aliases: ["color", "colour", "color_grade", "color grade"] },
  ...
  { key: "cert_lab", label: "Cert lab", type: "enum", enumValues: CERT_LABS, aliases: ["lab", "certificate_lab", "cert"] },
  { key: "cert_number", label: "Cert number", type: "string", aliases: ["cert_no", "certificate", "certificate_number", "report_no", "report_number"] },
...
export function suggestMapping(headers: string[]): Record<string, string | "__skip__"> {
  ...
  for (const h of headers) {
    ...
    if (aliased) { out[h] = aliased.key; continue; }
...
export function validateMappedRow(
  ...
  if (mapped.carat_weight !== undefined && mapped.carat_weight !== "" && (isNaN(carat) || carat < 0.01 || carat > 100)) {
...
export function buildTemplateCsv(): string {
  ...
  const example: Record<string, string> = {
    stone_type: "diamond", shape: "round", carat_weight: "1.05", wholesale_price_usd: "4200",
    colour_grade: "F", clarity_grade: "VS1", cut_grade: "Excellent",
    polish: "Excellent", symmetry: "Excellent", fluorescence: "None",
    cert_lab: "GIA", cert_number: "2191234567", origin: "natural",
    country_of_origin: "Botswana", treatment: "none",
    measurements_length: "6.52", measurements_width: "6.55", measurements_height: "4.03",
    depth_pct: "61.6", table_pct: "57", available_qty: "1", listing_type: "single",
