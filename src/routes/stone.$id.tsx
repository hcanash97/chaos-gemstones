...
export const Route = createFileRoute("/stone/$id")({
  ...
  loader: async ({ params }) => {
    const { data } = await supabase
      .from("stones")
      .select(
        "id, stone_type, shape, carat_weight, colour_grade, clarity_grade, cert_lab, cert_number, treatment, country_of_origin, wholesale_price_usd, price_currency, status, stone_images(storage_url, external_image_url), profiles:dealer_id(company_name, city, country)",
      )
      .eq("id", params.id)
      .maybeSingle();
  ...
  head: ({ loaderData, params }) => {
    ...
    const img = s.stone_images?.[0]?.storage_url || s.stone_images?.[0]?.external_image_url;
    const availability =
      s.status === "available"
